...
impl AssetBundleLoader {
    ...
    pub async fn load_from_directory(path: PathBuf) -> Result<Self, ()> {
        let path = match tokio::fs::canonicalize(&path).await {
            ...
            Err(canon_error) => {
                tracing::error!(
                    "Error canonicalizing asset directory path: {} {}",
        ...
        let load_order_file = match tokio::fs::read_to_string(&load_order_file_path).await {
            ...
            Err(read_error) => {
                tracing::error!(
                    "Error loading asset load order file from path: {} {}",
                    load_order_file_path.to_string_lossy(),
                    read_error
                );
                return Err(());
            }
        ...
        let load_order_file = match serde_json::de::from_str::<LoadOrderFile>(&load_order_file) {
            ...
            Err(invalid_format) => {
                tracing::error!(
                    "Asset load order file at {} is invalid format: {}",
                    load_order_file_path.to_string_lossy(),
                    invalid_format
                );
                return Err(());
        ...
        let tasks = load_order_file
            .files
            .iter()
            .map(|asset_file| (asset_file, &path))
            .map(|(asset_file, path)| async move {
                let asset_file = match tokio::fs::canonicalize(path.join(asset_file)).await {
                    Ok(canonicalized) => canonicalized,
                    Err(canon_error) => {
                        tracing::error!(
                            "Error canonicalizing asset path: {} {}",
                            asset_file.to_string_lossy(),
                            canon_error
                        );
                        return Err(());
                    }
                ...
                if asset_file.starts_with(&path) == false {
                    tracing::error!(
                        "Asset bundle path is outside specified config path: {}",
                        asset_file.to_string_lossy()
                    );
                    return Err(());
                }
                ...
                let metadata = match tokio::fs::metadata(&asset_file).await {
                    ...
                    Err(bad_file) => {
                        tracing::error!(
                            "Asset bundle information could not be loaded for: {} with error {}",
                            asset_file.to_string_lossy(),
                            bad_file
                        );
                        return Err(());
                ...
                let bundle_type = match metadata.is_file() {
                    true => match asset_file.extension() {
                        Some(extension) => {
                            let file_type = &&*extension.to_string_lossy();
                            match AssetBundleFileType::from_file_extension(&file_type) {
                                Some(valid) => AssetBundleType::Zipped(valid),
                                None => {
                                    tracing::error!(
                                        "Asset file has an unsupported extension: {}",
                                        asset_file.to_string_lossy()
                                    );
                                    return Err(());
                                }
                        ...
                        None => {
                            tracing::error!(
                                "Asset file does not have an extension, which is required: {}",
                                asset_file.to_string_lossy()
                            );
                            return Err(());
                        }
                    },
                    false => AssetBundleType::Folder,
                };
                ...
                let name = match asset_file.file_stem() {
                    ...
                    None => {
                        tracing::error!(
                            "Asset bundle does not have a name, which is required: {}",
                            asset_file.to_string_lossy()
                        );
                        return Err(());
                    }
        ...
        match error_loading {
            true => {
                tracing::error!(
                    "Error loading asset bundles from file {}",
                    path.to_string_lossy()
                );
                Err(())
            }
            false => Ok(AssetBundleLoader { bundles: loaded }),
        }
    }
}
