...
use crate::{
    backend::{
        ...
        world_objects::{
            components::{
                collision_component::CollidableComponent, player_controlled_component::PlayerControlledComponent, semi_newtonian_physics_component::SemiNewtonianPhysicsComponent
            },
        ...
        let following_id = match session.should_follow {
            ...
            None => {
                let new_ship = ShipBundle::new(
                    &format!("Player Ship {}", session.remote_address),
